        ...
        <tbody>
            @foreach ($partenaires as $partenaire)
                <tr
                    ...
                    <td class="px-6 py-4">
                        @switch($partenaire->status)
                            @case('masquer')
                                <span class=" px-2 py-1 rounded-full" style="background-color: yellow;">{{ $partenaire->status }}</span>
                                @break
                            @case('afficher')
                                <span class=" px-2 py-1 rounded-full" style="background-color: green;">{{ $partenaire->status }}</span>
                                @break
                    ...
                    <td class="flex items-center px-6 py-4 space-x-3">
                        <a href="show-partenaire-update/{{ $partenaire->id }}"  wire:loading.attr="disabled"
                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Modifier</a>
                        <a href="#" wire:click="delete({{ $partenaire }})" wire:loading.attr="disabled"
